...
# The home modules resolve their mkOutOfStoreSymlink paths through ~/.dotfiles,
...
# normally and records the profile in /etc/dotfiles-profile.
# sudo resets PATH to a secure default that excludes /nix/.../bin, so resolve
# the absolute path of `nix` first and invoke that.
NIX_BIN="$(command -v nix)"
sudo "$NIX_BIN" run ~/.dotfiles#darwin-rebuild -- \
...
# hook's closure. `direnv allow` trusts the .envrc; `direnv exec ... true` forces
# the first load now, non-interactively. || true: a hook hiccup must not abort bootstrap.
if command -v direnv >/dev/null 2>&1; then
  direnv allow "$HOME/.dotfiles" || true
  direnv exec "$HOME/.dotfiles" true || true
else
  # direnv not on PATH in this shell yet (new terminal needed to pick up the
  # home-manager profile) - install the hook transiently now; it becomes GC-proof
  # once the user runs `direnv allow` after cd-ing into ~/.dotfiles in a new terminal.
...
echo "==> Done. Use 'rebuild' (or ./rebuild.sh) for future changes."
