...
`home/` files (Neovim, WezTerm, herdr, Claude settings) are live-symlinked - editing them takes effect immediately without running `rebuild.sh`. Only run rebuild when changing package lists, system defaults, or shell config in `.nix` files.
...
`home/AGENTS.md` is the shared agent policy file - it is symlinked to `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, and `~/.config/opencode/AGENTS.md`.
